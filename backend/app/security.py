...
def hash_password(password: str) -> str:
    """Returns a self-describing hash string: algo$iterations$salt$hash
    (all hex), so iteration count can change later without breaking
    existing stored hashes."""
    salt = secrets.token_hex(16)
    digest = hashlib.pbkdf2_hmac(PBKDF2_ALGO, password.encode("utf-8"), bytes.fromhex(salt), PBKDF2_ITERATIONS)
...
def verify_password(password: str, stored_hash: str) -> bool:
    try:
        ...
        return False
    candidate = hashlib.pbkdf2_hmac(algo, password.encode("utf-8"), bytes.fromhex(salt), iterations)
...
def create_access_token(admin: models.AdminUser) -> tuple[str, int]:
    ...
    expires_delta = dt.timedelta(minutes=settings.jwt_expires_minutes)
...
def get_current_admin(request: Request, db: Session = Depends(get_db)) -> models.AdminUser:
    """Resolves the logged-in admin from the `Authorization: Bearer <jwt>`
    header. Use as a route dependency to require any admin login."""
    token = _extract_bearer_token(request)
    if not token:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Admin login required. Include 'Authorization: Bearer <token>'.",
        )
    payload = decode_access_token(token)
    admin_id = payload.get("sub")
    admin = db.get(models.AdminUser, int(admin_id)) if admin_id else None
    if not admin or not admin.is_active:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="This admin account is no longer active. Please log in again.",
        )
    return admin


# Backwards-compatible alias: existing routers depend on `require_admin`.
require_admin = get_current_admin


def require_superadmin(admin: models.AdminUser = Depends(get_current_admin)) -> models.AdminUser:
    """Use for endpoints that manage other admin accounts."""
...
def get_optional_admin(request: Request, db: Session = Depends(get_db)) -> Optional[models.AdminUser]:
    """Like get_current_admin, but returns None instead of raising when no
    (or an invalid) session is present. Useful for endpoints that behave
    slightly differently for logged-in organizers without requiring it."""
    token = _extract_bearer_token(request)
    if not token:
        return None
    try:

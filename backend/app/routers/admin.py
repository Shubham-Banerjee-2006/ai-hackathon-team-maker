...
bulk CSV import, and a quick "clear all teams" reset.
...
def get_stats(
    ...
    for participant in participants:
        skill_counter.update(csv_to_list(participant.skills))
        domain_counter.update(csv_to_list(participant.domains))

        if participant.preferred_role:
            role_counter[participant.preferred_role] += 1

        if participant.experience_level:
            experience_counter[participant.experience_level] += 1

    def top(
        counter: Counter,
        n: int = 8,
    ) -> List[schemas.DistributionEntry]:
        return [
            schemas.DistributionEntry(
                label=label,
                count=count,
            )
    ...
    avg_coverage = (
        sum(team.skill_coverage_score for team in teams) / len(teams)
        if teams
        else 0.0
    )

    avg_compat = (
        sum(team.compatibility_score for team in teams) / len(teams)
        if teams
        else 0.0
    )
    ...
    return schemas.StatsOut(
        ...
        recent_actions=[
            schemas.AuditLogOut.model_validate(action)
...
def get_audit_log(
    ...
    return [
        schemas.AuditLogOut.model_validate(entry)
...
def reset_teams(
    ...
    Useful when an organizer wants a clean slate before re-registering
    people or changing team size.
    """
    ...
    log_action(
        ...
        admin.username,
        "teams.reset",
        f"{admin.username} cleared {count} team(s).",
    )
...
def import_participants_csv(
    ...
    """
    Bulk-register participants from a CSV.
    ...
    Skills and domains can be separated using:
        |
        ;
        ,
    ...
    existing_emails = {
        email
        for (email,) in db.query(models.Participant.email).all()
    }

    def split_multi(value: str) -> list[str]:
        """
        Supports pipe, semicolon, and comma-separated values.
        """
    ...
    for row_number, row in enumerate(
        ...
        try:
            participant = models.Participant(
                ...
                email=email,
                skills=list_to_csv(
                    split_multi(row.get("skills", ""))
                ),
                domains=list_to_csv(
                    split_multi(row.get("domains", ""))
                ),
                preferred_role=row.get(
                    "preferred_role",
                    "",
                ),
                experience_level=(
    ...
    log_action(
        db,
        admin.username,
        "participants.import_csv",
        (
            f"{admin.username} imported "
            f"{created} participant(s) via CSV "
